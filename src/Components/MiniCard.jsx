...
export default function MiniCard() {
  ...
  useEffect(() => {
    dispatch(weatherDays({ cityName: "ifrane" })); // Replace with the default or dynamic city name
  }, [dispatch]);
  ...
  const cards = Object.keys(forecast)
    ...
    .map((day, index) => {
      ...
      const avgTemp = Math.round(
        dayData.reduce((acc, item) => acc + item.temperature, 0) / dayData.length
      );
    ...
    return (
      <div className="w-full mx-auto py-4">
        {/* Add the title */}
        <h2 className="text-start text-4xl pl-8 text-white font-bold mb-4">Next 5 Days</h2>
        <div className="daysCard flex flex-wrap lg:flex-wrap gap-x-4">{cards}</div>
      </div>
    );
  }
